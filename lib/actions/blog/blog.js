...
export const generateBlogPost = async (prompt, blogParametersId) => {
  ...
  console.log("Collection name:", BlogPost.collection.name);
  console.log("DB name:", BlogPost.db.name);
...
export const deleteBlogPost = async (blogPostId) => {
  console.log("deleteBlogPost called with id:", blogPostId);
  await dbConnect();
  const blogPost = await BlogPost.findById(blogPostId);
  if (!blogPost) {
    throw new Error("Blog post not found.");
  }
  console.log("delete blog post", blogPost.theme);
  const result = await BlogPost.deleteOne({ _id: blogPostId });
  console.log("Delete blog post result:", result, blogPostId);
