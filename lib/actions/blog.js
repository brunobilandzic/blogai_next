...
const processGenerateAuth = (userRole) => {
...
export const generateBlogPost = async (blogParametersId, { signal }) => {
  const startTime = Date.now();
  await dbConnect();
  console.log("Generating blog post for parameters ID:", blogParametersId);
  const { userRole } = await sessionUserRoleServer();
  const blogParameters = await BlogParameters.findById(
    blogParametersId
  ).populate("chaptersParameters");

  processGenerateAuth(userRole);

  if (
    !blogParameters.prompt.promptText ||
    blogParameters.prompt.promptText.trim() === ""
  ) {
    throw new Error("Prompt text is empty.");
  }

  const response = await fetchResponse(blogParameters.prompt.promptText, {
    signal,
  });
  console.log("fetched response length:", response.length);
...
export const deleteBlogPost = async (blogPostId) => {
  ...
  const blogParameters = await BlogParameters.findOne({
    blogPost: blogPostId,
  }).populate("chaptersParameters");
  if (!blogParameters) {
