...
export const generateBlogPost = async (blogParametersId) => {
  ...
  if (!blogParameters.promptText || blogParameters.promptText.trim() === "") {
    alert("Please enter a prompt.");
    return;
  }

  const response = await fetchResponse(blogParameters.promptText);
...
export const deleteBlogPost = async (blogPostId) => {
  console.log("Deleting old blog post in action:", blogPostId);
  if (!blogPostId) {
    return;
  }
  await dbConnect();
