...
export const generateBlogParams = async (
  ...
  for (const pd of paramsDescs) {
    generatePromises.push(generateFromDesc(pd, { signal }));
  }
  const blogParameters = await Promise.all(generatePromises);
  return clean(blogParameters);
};

const generateFromDesc = async (paramsDesc, { signal }) => {
  return new Promise(async (resolve, reject) => {
    try {
      const blogParameters = await createBlogParameters(paramsDesc, { signal });
