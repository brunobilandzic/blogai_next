...
export const generateBlogParams = async (
  ...
  let paramsDescs = [];
  for (const paramsDesc of _paramsDescs) {
    paramsDescs.push({
      paramsDesc,
  ...
  try {
    for (const pd of paramsDescs) {
      console.log("Prompt for generating blog parameters:", pd.prompt);
    }
  } catch (error) {
    return {
      blogParameters: null,
...
const buildParamsPrompt = async (paramsDesc) => {
  return `Tvoj je zadatak generirati json dokument s poljima kao iz primjera ${getParamExample()} Ovo je tema blog poasta: ${
    ...
    paramsDesc.audience
  }. Tvoj je zadatak napisati json stringified dokument koji kasnije mogu parsirati pomocu JSON.parse funkcije u JavaScriptu. Vrati samo validan JSON bez dodatnih objašnjenja. Sve vrijednosti napiši na hrvatskom jeziku.`;
};
