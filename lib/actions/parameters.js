...
import { testBlogParamsDescs33 } from "@/components/UI/forms/constants";
...
export const generateBlogParams = async (paramsDescs = testBlogParamsDescs) => {
  try {
    const prompt = await buildParamsPrompt(paramsDesc);
...
const buildParamsPrompt = async (paramsDesc) => {
  return `Ovo je tema blog poasta: ${paramsDesc.theme}. Publika za koju je blog post namijenjen je: ${paramsDesc.audience}. Stil pisanja treba biti: ${paramsDesc.writingStyle}. Dodatne upute su: ${paramsDesc.additionalInstructions}. Napiši parametre za kreiranje blog posta u JSON formatu sa sljedećim poljima: "title", "tone", "length", "keywords" i "chaptersParameters" (niz objekata sa poljima "chapterTitle" i "chapterLength").`;
};
