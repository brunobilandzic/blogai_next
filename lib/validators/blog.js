...
const paramsPromptSchema = Joi.object({
  theme: Joi.string().min(3).max(200).required(),
  description: Joi.string().min(0).max(1000).required(),
  audience: Joi.string().min(3).max(200).required(),
});
export function validateParamsPrompt(paramsPrompt) {
  return paramsPromptSchema.validate(paramsPrompt, { allowUnknown: true });
}
